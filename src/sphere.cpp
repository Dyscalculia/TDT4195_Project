...
unsigned int createCircleVAO(unsigned int slices, unsigned int layers, float r, float g, float b, float a) {
	...
	unsigned int vertexCount = triangleCount * VERTICES_PER_TRIANGLE * COMPONENTS_PER_VERTEX;
	unsigned int colorCount = triangleCount * VERTICES_PER_TRIANGLE * 4;
	unsigned int indicesCount = triangleCount * VERTICES_PER_TRIANGLE;
	...
	unsigned int vao_id = CreateVAOWithTriangles(vertices, colors, indices, vertexCount, colorCount, indicesCount);
